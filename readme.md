...
- 問い合わせ内容の DB 保存をテーブルを増やさずに実現する方法を検討
...
- ~~メール内容は DB に保存しない~~ → DB 保存できるように変更
- ファイルは送信できない
 ...
 */
add_filter('omf_data_custom_field_key', function ($field_key) {
  if ($field_key === 'custom') {
    return 'カスタマイズ';
  }
...
- 移行作業
  ...
  2. プラグインをインストール
  3. 「メールフォーム」からフォームを作成し、メール文面などを設定
